...
| [dsh-geek-header](dsh-geek-header/) | v0.4.6 | 页头重排分段条 + 第三方插件区 + 会话标题一键生成 |
...
## dsh-geek-header · 页头重排（v0.4.6）

第一行第三方插件区（`geekUiHeader` 服务），第二行收纳页签/谱系/动作平铺成统一分段条，面包屑文字与 Session log 隐藏，拥挤时动作折叠进 ⋯ 菜单；「生成标题」读整段对话、用你当前选中的模型生成会话标题，生成中/成功/失败都有即时反馈。
