...
DSH Web 会话页头重排插件：第一行第三方插件区（`geekUiHeader` 服务），第二行把页签/谱系/动作收纳成一条统一分段条；提供 **「生成标题」** 与 **「系统提示词」** 查看（会话流内 System prompt 行默认隐藏）。
...
- **第一行**：第三方插件区，官方不放任何内容。
- **第二行分段条**：页签 + 生成标题 + 系统提示词 + 子代理谱系 + actions 槽各项，全部压平成统一格子
...
- **隐藏**：面包屑标题文字与 `/` 分隔符；utilities 槽（Session log 按钮）整段隐藏，导出走 `/export` 命令；
  会话流内平台 `System prompt` 折叠行（`data-chat-flow-kind="system-prompt"`）整段隐藏，改由页头按钮查看。
- **生成标题**：点击后按整段对话用当前选中的模型生成会话标题；生成中/成功/失败均有即时反馈，空会话禁用。
- **系统提示词**：点击展开下拉，展示当前会话 `requestHeader().system`（完整 model-facing system 字符串，含插件 section 如破甲；不含上下文注入与工具 schema）。破甲等变更需等下一次模型请求写入 header 后刷新；空会话禁用。
